...
class ModelManager:
    ...
    def __init__(self, model_name: Optional[str] = None):
        ...
        if (
            ...
            and settings.gpu_memory_fraction < 1.0
            ...
            try:
                ...
                logger.warning(
                    f"Failed to enable quantization: {e}. Proceeding without quantization."
                )
    ...
    def _determine_device(self) -> str:
        ...
        if torch.cuda.is_available():
            ...
            logger.info(
                f"GPU detected: {torch.cuda.get_device_name(0)} ({gpu_memory_gb:.1f}GB)"
            )
            ...
            if gpu_memory_gb >= 8.0:  # Minimum recommended for VL models
                ...
                logger.warning(
                    f"GPU memory ({gpu_memory_gb:.1f}GB) may be insufficient for optimal performance"
                )
    ...
    async def load_model(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                try:
                    # Suppress warnings by explicitly setting use_fast=False for compatibility
                    # (A.X-4.0-VL-Ligth use slow image processor)
                    import warnings

                    with warnings.catch_warnings():
                        warnings.filterwarnings("ignore", category=FutureWarning)
                        warnings.filterwarnings(
                            "ignore", message=".*image_processor_class.*"
                        )
                        warnings.filterwarnings(
                            "ignore", message=".*slow image processor.*"
                        )
                    ...
                    logger.warning(
                        f"Could not load processor: {e}. Using tokenizer only."
                    )
                    self.processor = None
                ...
                logger.info(
                    f"Model {self.model_name} loaded successfully in {self.load_time:.2f}s"
                )
                ...
                if (
                    ...
                    logger.info(
                        "Chat template available - using tokenizer's chat template"
                    )
                else:
                ...
                # Log special tokens
                logger.info(
                    f"Special tokens - EOS: {self.tokenizer.eos_token}, PAD: {self.tokenizer.pad_token}"
                )
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        health_status = {"healthy": False, "status": self.status.value, "checks": {}}
        ...
        try:
            ...
            if self.model:
                try:
                    ...
                    health_status["checks"]["model_on_device"] = (
                        str(model_device) == self.device or self.device == "auto"
                    )
            ...
            if self.device.startswith("cuda") and torch.cuda.is_available():
                ...
                health_status["checks"]["gpu_memory_usage_percent"] = (
                    memory_usage_percent
                )
                health_status["checks"]["gpu_memory_healthy"] = (
